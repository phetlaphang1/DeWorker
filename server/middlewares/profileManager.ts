...
export async function initializeProfiles() {
  try {    
    ...
    setInterval(autoRunProfile, (config.INTERVAL_OF_AUTO_RUN_PROFILE || 60) * 1000);
...
export async function createProfileFolder(profile: Profile): Promise<string> {
  ...
  try {
    ...
    configData.created = new Date(profile.created).toISOString(),
    ...
    const scriptPath = path.join(config.ORIGINAL_CWD,"server","scripts","samples");
    let scriptContent = `// Browser Automation Script for ${profile.name}\n`;
    if(config.IS_TWITTER_CARRING){
      scriptContent += await fsPromises.readFile(path.join(scriptPath, 'twitterCarring.js'), 'utf8');
    }else{
      scriptContent += await fsPromises.readFile(path.join(scriptPath, 'example.js'), 'utf8');
    }
...
export async function updateProfileStatusInConfig(profileId: number, status: string) {
  try {
...
let availProfiles = new Array<number>();
async function autoRunProfile() {  
  
  if (config.IS_AUTO_RUN_PROFILE != true) {return}
  console.log("start auto run...");
  try {      
    if (await utils.getRunningProfileCount() >= config.PARRALEL_RUNNING_PROFILE) {
