...
import { getTaskPathFromTaskId } from "../middlewares/utils";
import { TASKS_PROFILE } from "../config";
...
export function registerTaskRoutes(app: Express) {
  ...
  app.post("/api/tasks/:id/run", async (req, res) => {
    ...
    // Check if task has dedicated profile or profile/script data
  ...
  app.post("/api/tasks/fetch-from-task-center", async (req, res) => {
    try {     
      const taskCenterTasks = await taskManager.loaddAllTasksFromCenter();      
      res.json({ 
  ...
  // Clear task data endpoint - removes profile folder and resets dedicatedProfileId
  app.post("/api/tasks/:id/clear", async (req, res) => {
    try {
      ...
      // const profileData = task.profileData as TaskCenterTask['profile'] | null;
      // const dedicatedProfileId = profileData?.dedicatedProfileId ?? null;

      // const folderName = task.id.toString();
      // const taskProfilePath = path.join(TASKS_PROFILE, folderName);
      
      // // Remove task profile folder if it exists
      // try {
      //   await fs.promises.rm(taskProfilePath, { recursive: true, force: true });
      //   console.log(`[Task ${taskId}] Removed profile folder: ${taskProfilePath}`);
      // } catch (removeError) {
      //   console.log(`[Task ${taskId}] No profile folder to remove or removal failed:`, (removeError as Error).message);
      // }

      // res.json({ 
      //   message: "Task data cleared successfully",
      //   clearedFolder: taskProfilePath,
      //   resetDedicatedProfile: !!dedicatedProfileId
      // });
  ...
  app.post("/api/tasks/:id/stop", async (req, res) => {
    try {
      ...
      await taskManager.updateTaskCenter(taskId.toString(), "FAILED", null);
  ...
  app.post("/api/tasks/:id/open-browser", async (req, res) => {
    try {
      ...
      if (dedicatedProfileId) {
        // Use dedicated profile folder
        const profileDir = path.join(TASKS_PROFILE, dedicatedProfileId.toString(), "chrome-profile");
        ...
        // Use task-specific profile folder
        const taskProfileDir = path.join(TASKS_PROFILE, task.profileId?.toString() || "", "chrome-profile");
